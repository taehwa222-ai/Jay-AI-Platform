...
일반 구성원의 접근 상태만 관리합니다. 신규 가입자는 모듈 권한 없이 생성되므로 승인 전에 주식
Lab과 Content Ops 중 필요한 권한을 명시적으로 선택합니다. 대표의 두 모듈 권한은 항상 활성화되고,
기존 계정은 배포 후에도 현재 접근 권한을 유지합니다. SQLite 데이터는
`DATA_DIR/jay_ai_platform.db`에 저장됩니다.
...
로그인은 이메일별 5회 연속 실패 시 15분 동안 제한됩니다. 사용자 관리 화면에서 계정별 전체
세션 종료와 감사 로그 확인이 가능하며, 대표만 다른 사용자의 비밀번호를 임시 비밀번호로
초기화할 수 있습니다. 비밀번호 변경·초기화와 세션 종료는 기존 인증 토큰을 즉시 무효화합니다.
