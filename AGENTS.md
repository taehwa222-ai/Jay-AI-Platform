...
**카카오톡 이모티콘 캐릭터 사업 — 폐기 (2026-09-24 대표 결정)**

제품(기본 32종 PNG)은 완성했으나 판매 채널이 없어 첫 수익까지의 거리를 산정할 수 없었다.
`/emo-pipeline` 과 `emo-*` 에이전트는 더 이상 실행하지 않는다. 파일·에이전트 정의는
재개 가능성을 위해 남겨 둔다. 사유·재개 조건은 `content/emoticon/_business-status.md` 참고.
...
- 첫 캐릭터 "곁곰(Gyeotgom)"이 `content/emoticon/gyeotgom/` 에 있다 — 정지형 32종 세트 2개
  (`set-basic-32.md`, `set-monday-32.md`)가 문구·상황 기준 3차 검수(2026-08-09)까지 통과했다.
  2026-08-15 대표가 **AI 제작 경로**를 선택해 AI 캐릭터 IP로 진행한다. AI 산출물은 SNS·굿즈·
  브랜드 활용용이며, 카카오 공식 운영 원칙의 생성형 AI 활용 제안 제한이 유지되는 동안 카카오
  제출은 보류한다. 사람 외주 문서는 향후 카카오 제출이 필요할 때 쓰는 대안 경로다.
- AI 시안은 `content/emoticon/gyeotgom/assets/ai-concepts/`에 보관할 수 있다. 최종 이미지·
  애니메이션 대량 파일은 용량을 고려해 별도 로컬/드라이브에 보관한다. 카카오 제출 규격과
  생성형 AI 정책은 시기에 따라 바뀔 수 있으므로 매번 공식 사이트를 재확인한다.
