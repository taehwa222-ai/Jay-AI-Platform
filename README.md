...
구성원의 접근 상태만 관리합니다. 새 가입자는 주식 Lab과 Content Ops 권한이 모두 꺼진 상태로
생성되며, 승인 화면에서 필요한 모듈만 별도로 허용합니다. 기존 사용자는 마이그레이션 시 현재
접근 권한을 유지합니다.

인증 보호 기능으로 이메일별 15분 동안 로그인 5회 실패 제한, 모든 세션 강제 종료, 본인 비밀번호
변경, 대표의 구성원 비밀번호 초기화와 사용자 관리 감사 로그를 제공합니다.
